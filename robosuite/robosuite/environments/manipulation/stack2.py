...
class Stack2(SingleArmEnv):
    ...
    def __init__(
        ...
        self.placement_initializerA = placement_initializer
        self.placement_initializerB = placement_initializer
    ...
    def _load_model(self):
        ...
        if self.placement_initializerA is not None:
            self.placement_initializerA.reset()
            self.placement_initializerA.add_objects(self.cubeA)
        else:
            self.placement_initializerA = UniformRandomSampler(
                name="ObjectSampler",
        ...
        if self.placement_initializerB is not None:
            self.placement_initializerB.reset()
            self.placement_initializerB.add_objects(self.cubeB)
        else:
            self.placement_initializerB = UniformRandomSampler(
                name="ObjectSampler",
                mujoco_objects=self.cubeB,
                x_range=[0.0499, 0.0501],  # x_range=[-0.0001, 0.0001],
                y_range=[0.049, 0.050],
                rotation=0,
                ensure_object_boundary_in_range=False,
                ensure_valid_placement=True,
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            objectA_placements = self.placement_initializerA.sample()
            objectB_placements = self.placement_initializerB.sample()

            # Loop through all objects and reset their positions
            for obj_pos, obj_quat, obj in objectA_placements.values():
                self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))
            for obj_pos, obj_quat, obj in objectB_placements.values():
                self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array(obj_pos), np.array(obj_quat)]))

...
# In this version, I try to strictly reproduce the network structure in the
...
class TanhWrappedDistribution(D.Distribution):
    ...
    def sample(self, sample_shape=torch.Size(), return_pretanh_value=False):
        ...
        """
        z = self.base_dist.sample(sample_shape=sample_shape)
        # .detach()
        print("prnt out the device", z.device)
...
class Resnet18SpatialSoftmax(nn.Module):
    ...
    def __init__(self, input_shape, image_latent_dim=64, num_kp=32, temperature=1., learnable_temperature=False):
    ...
    def forward(self, x):
        ...
        feature_keypoints = self.fc(feature_keypoints)

        return feature_keypoints


class GMM_MLP(nn.Module):
    def __init__(self, input_dim, output_dim=7, hidden_dims=[1024, 1024], num_modes=5, min_std=0.01, std_activation='softplus', low_noise_eval=True, use_tanh=False):
    ...
    def forward(self, robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos, emedded_robot0_eye_in_hand_image, emedded_agentview_image):
        x = torch.cat([robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos, emedded_robot0_eye_in_hand_image, emedded_agentview_image], 1)
...
class PiNetwork(nn.Module):
    def __init__(self, input_shape, image_latent_dim, action_dim, low_dim_input_dim, mlp_hidden_dims):
        # env_params['action_max'] in gym is now env.action_spec[1], while [0] is action_min
        super(PiNetwork, self).__init__()

        self.RGBView1ResnetEmbed = Resnet18SpatialSoftmax(input_shape, image_latent_dim)
        self.RGBView3ResnetEmbed = Resnet18SpatialSoftmax(input_shape, image_latent_dim)
        self.Probot = GMM_MLP(input_dim=low_dim_input_dim + 2 * image_latent_dim, output_dim=action_dim, hidden_dims=mlp_hidden_dims)
...
class Imageonly_Model(nn.Module):
    def __init__(self, original_model, fixed_args):
        super(Imageonly_Model, self).__init__()
        self.original_model = original_model
        self.fixed_args = fixed_args

    def forward(self, view1_image):
        # Use the provided fixed arguments and pass agentview_image to the original model
        robot0_eef_pos, robot0_eef_quat, robot0_gripper_qpos, view2_image = self.fixed_args
        action = self.original_model(

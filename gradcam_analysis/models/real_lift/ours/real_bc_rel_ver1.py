...
from nn_modules.resnet18_gmmmlp_view13rgb_rel_model_low_dim_layer import PiNetwork
...
if args.vision1 == 'agent_view':
    vision1_anchors = np.load('hdf5_image/real_lift/real_' + args.vision1 + '_' +str(args.anchor_num) + 'anchor_images.npy')
else:
    vision1_anchors = np.load('hdf5_image/real_lift/real_' + args.vision1 + '_' +str(args.anchor_num) + 'anchor_images_from_agent_view_idx.npy')

if args.vision2 == 'agent_view':
    vision2_anchors = np.load('hdf5_image/real_lift/real_' + args.vision2 + '_' +str(args.anchor_num) + 'anchor_images.npy')
else:
    vision2_anchors = np.load('hdf5_image/real_lift/real_' + args.vision2 + '_' +str(args.anchor_num) + 'anchor_images_from_agent_view_idx.npy')
...
data_loader = DataLoader(dataset=dataset, sampler=None, batch_size=16, shuffle=True, num_workers=2, drop_last=True)
...
if args.log:
    writer = SummaryWriter('training_data/real_lift/real_bc_rel_ver1_vision1' + args.vision1 + '_vision2' + args.vision2 + '_anchors' + str(args.anchor_num) + '_lr' + str(args.lr) + '_seed' + str(args.seed))
...
if args.save_model:
    ...
    if not os.path.exists(model_path):
        os.mkdir(model_path)
    model_file_name = 'real_bc_rel_ver1_vision1' + args.vision1 + '_vision2' + args.vision2 + '_anchors' + str(args.anchor_num) + '_lr' + str(args.lr) + '_seed' + str(args.seed) + '_model.pt'

...
class PiNetwork(nn.Module):
    def __init__(self, input_shape, image_latent_dim, action_dim, low_dim_input_dim, rnn_hidden_dim):
    ...
    def forward_train(self, seq_agentview_images, seq_robot0_eye_in_hand_images, seq_eef_pos, seq_eef_quat, seq_gripper_qpos, rnn_init_state=None, return_state=False):
        ...
        # embedded_images = torch.zeros(batch_size, seq_len, 2 * self.image_latent_dim, device=seq_agentview_images.device)
        ...
        #     embedded_view1 = self.RGBView1ResnetEmbed(seq_agentview_images[:, t])
        #     embedded_view3 = self.RGBView3ResnetEmbed(seq_robot0_eye_in_hand_images[:, t])
        #     embedded_images[:, t] = torch.cat([embedded_view1, embedded_view3], dim=1)

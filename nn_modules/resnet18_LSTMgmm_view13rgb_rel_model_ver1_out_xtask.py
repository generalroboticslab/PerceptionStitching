...
class Resnet18SpatialSoftmax(nn.Module):
    ...
    def forward(self, x):
        ...
        return relative_interface, x_task
...
class PiNetwork(nn.Module):
    ...
    def forward_train(self, seq_agentview_images, seq_robot0_eye_in_hand_images, seq_eef_pos, seq_eef_quat, seq_gripper_qpos, rnn_init_state=None, return_state=False):
        ...
        embedded_view1, view1_xtask = self.RGBView1ResnetEmbed(seq_agentview_images)
        embedded_view3, view2_xtask = self.RGBView3ResnetEmbed(seq_robot0_eye_in_hand_images)
    ...
    def forward(self, seq_agentview_images, seq_robot0_eye_in_hand_images, seq_eef_pos, seq_eef_quat, seq_gripper_qpos, rnn_init_state=None, return_state=False):
        ...
        """
        if return_state:
            action_dist, rnn_state, _, _ = self.forward_train(seq_agentview_images, seq_robot0_eye_in_hand_images, seq_eef_pos, seq_eef_quat, seq_gripper_qpos, rnn_init_state=rnn_init_state, return_state=return_state)
            return action_dist.sample(), rnn_state
        else:
            action_dist, _, _ = self.forward_train(seq_agentview_images, seq_robot0_eye_in_hand_images, seq_eef_pos, seq_eef_quat, seq_gripper_qpos, rnn_init_state=rnn_init_state, return_state=return_state)
